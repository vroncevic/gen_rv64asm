...
Info
    Validator for the gen_rv64asm bundle options.
'''
...
class GenRV64ASMBundleOptionsValidator:
    '''
        Validator for the gen_rv64asm bundle options.
        ...
        It defines:
            ...
            :methods:
                | validate - Validates the gen_rv64asm bundle options.
                | is_valid - Checks if the gen_rv64asm bundle options is valid.
    '''

    @classmethod
    def validate(cls, options: GenRV64ASMBundleOptions) -> None:
        '''
            Validates the gen_rv64asm bundle options.

            :param options: The gen_rv64asm bundle options to be validated.
            :exceptions:
        ...
        '''
        ctx: str = 'gen_rv64asm_bundle_options_validator::validate(...)'
        msg_options_none: str = 'the gen_rv64asm bundle options must be provided'
        msg_options_istype: str = 'the gen_rv64asm bundle options must be a Mapping'

        not_none(options, ctx, msg_options_none)
        istype(options, Mapping, ctx, msg_options_istype)

        for attr_name, expected_type in GenRV64ASMBundleKeys.get_option_to_type().items():
    ...
    @classmethod
    def is_valid(cls, options: GenRV64ASMBundleOptions) -> bool:
        '''
            Checks if the gen_rv64asm bundle options is valid.

            :param options: The gen_rv64asm bundle options to be checked.
